...
def test_estimate_is_free_and_states_the_cost() -> None:
    body = client.get(
        "/api/ai/estimate", params={"feature": "poster", "batch": True}
    ).json()
    ...
    assert body["model"] == ai.model_for("poster")


def test_estimate_shows_batch_is_half_price() -> None:
    def quote(batch: bool) -> dict[str, Any]:
        return client.get(
            "/api/ai/estimate", params={"feature": "poster", "batch": batch}
        ).json()
...
def test_prompts_are_seeded_and_described() -> None:
    ...
    assert {"poster-concept", "photo-edit"} <= {s["key"] for s in body["scopes"]}
